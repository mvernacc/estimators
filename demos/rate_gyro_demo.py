...
def main(args):
    ...
    if args.s:
        # Save to a pickle
        filename = "rate_gyro_test_{:s}.p".format(time.strftime("%Y-%m-%d_%H-%M-%S"))
        with open(filename, "wb") as f:
            pickle.dump({"y": y, "y_noisy": y_noisy, "bias": bias, "rate_gyro": rg}, f)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Test the rate_gyro; optionally save the data to a pickle for later Allan Variance testing."
    )
    parser.add_argument(
        "-s", action="store_true", help="Save the data to a pickle file."
    )
    parser.add_argument(
        "-n", "--n_steps", type=int, help="Number of timesteps.", default=1000
    )
