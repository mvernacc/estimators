...
def objective_function(x, mag_data):
    ...
    for i in range(mag_data.shape[0]):
        post_cal = np.squeeze(np.dot(np.eye(3) + D, np.array([mag_data[i]]).T)) - b
...
def main(args):
    ...
    if args.solver == "leastsq":
        ...
        magcal = magnetometer.MagCalUKF(
            mag.noise_cov, np.linalg.norm(mag.h_earth_ned), b=b_init, D=D_init
        )
        ...
        for i in range(3):
            if args.meas_source == "sim":
                plt.axhline(
                    y=mag.b[i], color=plot_colors[i], label="b[{:d}] true".format(i)
                )
        ...
        index_map = {0: (0, 0), 1: (1, 1), 2: (2, 2), 3: (0, 1), 4: (0, 2), 5: (1, 2)}
        for i in range(6):
            if args.meas_source == "sim":
                plt.axhline(
                    y=mag.D[index_map[i]],
            ...
            plot_single_state_vs_time(
                ...
                i + 3,
                color=plot_colors[i],
                label="D[{:d},{:d}] est".format(*index_map[i]),
                linestyle="--",
            )
        plt.xlabel("Step")
