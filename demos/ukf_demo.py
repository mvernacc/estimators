...
def main():
    ...
    sim_sensors = KalmanSensors(
        [gps_sim, baro_sim, magneto_sim], [[0, 1, 2], [2], [3]], n_system_states
    )
    ...
    x_est_init = np.concatenate(([100, -100, -500, 1], np.zeros(n_sensor_states)))
    ...
    for i in range(1, n_steps):
        ...
        x_soln = odeint(continuous_dubin_parafoil, x_traj[i - 1], [0, dt], args=(0.01,))
        x_traj[i] = x_soln[-1]
    ...
    plot_single_state_vs_time(
        ax, t_traj, x_est_traj, Q_traj, 2, color="red", label="est"
    )
    plt.xlabel("time")
    ...
    plot_single_state_vs_time(
        ax, t_traj, x_est_traj, Q_traj, 4, color="red", label="est"
    )
    plt.xlabel("time")
    ...
    for i in range(len(t_traj)):
        ...
        h_bias_sensor[i] = magneto_sim.h_bias_sensor + quat.rotate_vector(
            magneto_sim.h_bias_ned, q_ned2sensor
        )
