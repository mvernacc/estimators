...
class RateGyro(StatefulSensor):
    def __init__(self, rate_noise_std_dev=5e-4, constant_bias=None,
            scale_factor=[0, 0, 0], correlation_time=7.0*3600,
            bias_walk_process_std_dev=4e-4, dt=1e-2):
        ...
        Arguments:
            ...
            sacle_factor (real 3-vector): The scale factor error on each axis
                [units: none].
        ...
        if constant_bias == None:
            ...
            # Bhatt et al [3].
            constant_bias = np.random.multivariate_normal(np.zeros(3),
                np.diag([np.deg2rad(0.2)]*3)**2)
        ...
        sensor_state_process_covariance = np.diag([2.0 / self.correlation_time \
            * self.bias_walk_process_std_dev**2]*3)
        # Convert to discrete time.
    ...
    def update_sensor_state(self, x):
        ...
        Returns:
            None.
        '''
        self.bias = self.sensor_state_transition(x=None, sensor_state=self.bias,
            noise=True)
