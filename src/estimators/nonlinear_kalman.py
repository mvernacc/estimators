...
class ExtendedKalmanFilter(object):
    ...
    def update_measurement(self, y):
        ...
        C = self.C(self.x_est)
        h = self.h
        K = np.dot(np.dot(Q, C.T), np.linalg.inv(np.dot(C, np.dot(Q, C.T)) + self.R))
        self.Q = np.dot(np.eye(Q.shape[0]) - np.dot(K, C), Q)
...
class UnscentedKalmanFilter(object):
    ...
    def propagate_dynamics(self, u):
        ...
        self.Q = self.get_Q_from_sigma_points(sigma_points_next, self.x_est) + self.W
    ...
    def update_measurement(self, y):
        ...
        Q_xy = self.get_Q_from_sigma_points(
            sigma_points_x, self.x_est, sigma_points_y, y_est
        )  # + self.R
        # Compute the Kalman gain.
        K = np.dot(Q_xy, np.linalg.inv(Q_yy))
        # Update the state estimate using the Kalman gain.
        self.x_est = self.x_est + np.squeeze(np.dot(K, np.array([y - y_est]).T))
    ...
    def get_Q_from_sigma_points(self, sp1, x1, sp2=None, x2=None):
        ...
        for i in range(2 * n):
            Q += np.dot(np.array([sp1[i] - x1]).T, np.array([sp2[i] - x2]))
        Q = Q / (2.0 * n)
        return Q
