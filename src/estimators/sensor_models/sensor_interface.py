...
class KalmanSensors:
    def __init__(
        ...
        n_system_states,
        sensor_state_indices=None,
        n_sensor_states=0,
        system_transition_function=None,
        system_process_covariance=None,
        ...
        self.noise_cov = scipy.linalg.block_diag(*[s.noise_cov for s in sensors])
        # Build the augmented (system state + sensor state) process noise
        # covariance matrix.
        if system_process_covariance is not None:
            assert (
                system_process_covariance.shape[0]
            ...
            assert (
                system_process_covariance.shape[1]
                == self.n_system_states - n_redundant_states
            ), (
                "The system_process_covariance matrix must be square with"
                + " dimension equal to the number of system states."
            ...
            self.augmented_process_covariance = scipy.linalg.block_diag(*cov_blocks)
            assert (
                self.augmented_process_covariance.shape[0]
                == self.n_system_states + self.n_sensor_states - n_redundant_states
            )
            assert (
                self.augmented_process_covariance.shape[1]
                == self.n_system_states + self.n_sensor_states - n_redundant_states
            )
    ...
    def measurement_function(self, x):
        ...
        if self.sensor_state_indices is not None:
            ...
            for sensor, sys_states, sen_states in zip(
                self.sensors, self.system_state_indices, self.sensor_state_indices
            ):
                if hasattr(sensor, "is_stateful") and sensor.is_stateful:
                    # Stateful sensor: measurement depends on system state and
                    # on sensor state.
                    y.append(sensor.measurement_function(x[sys_states], x[sen_states]))
                else:
    ...
    def augmented_transition_function(self, x, u):
        ...
        if self.sensor_state_indices is not None:
            ...
            for sensor, sys_states, sen_states in zip(
                ...
                if hasattr(sensor, "is_stateful") and sensor.is_stateful:
                    x_next.append(
                        sensor.sensor_state_transition(x[sys_states], x[sen_states])
                    )
